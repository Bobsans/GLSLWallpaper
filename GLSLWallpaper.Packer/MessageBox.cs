...
static partial class UnsafeNativeMethods {
    ...
    internal static partial MessageBoxResult MessageBoxW(IntPtr hWnd, string text, string caption, MessageBoxStyle options);
}

public static class MessageBox {
    static string GetTitleFromAssembly(Assembly callingAssembly) {
        try {
            return callingAssembly.GetName().Name!;
        } catch (SecurityException) {
            return callingAssembly.FullName!.Split(',')[0];
        }
    }

    public static MessageBoxResult Show(string text, string? caption = null, MessageBoxStyle options = MessageBoxStyle.OkOnly) {
        if (string.IsNullOrEmpty(caption)) {
            caption = GetTitleFromAssembly(Assembly.GetCallingAssembly());
        }

        return UnsafeNativeMethods.MessageBoxW(IntPtr.Zero, text, caption, options);
    }
}
