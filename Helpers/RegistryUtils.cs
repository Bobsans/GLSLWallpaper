...
namespace GLSLWallpapers.Helpers {
    public static class RegistryUtils {
        ...
        static T Get<T>(RegistryKey root, string path, string key, T @default) {
            using RegistryKey appKey = root.OpenSubKey(path);
            object value = appKey?.GetValue(key);
            return value != null ? typeof(T) == typeof(object) ? (T)value : (T)Convert.ChangeType(value, typeof(T)) : @default;
        }
        ...
        public static T GetConfig<T>(string key, T @default) {
            return Get(Registry.CurrentUser, $"Software\\{Reference.NAME}", key, @default);
        }
