...
#define sp(t) pal((t), vec3(0.26, 0.76, 0.77), vec3(1, 0.3, 1), vec3(0.8, 0.4, 0.7), vec3(0, 0.12, 0.54))
#define hue(v) (0.6 + 0.76 * cos(6.3 * (v) + vec4(0, 23, 21, 0)))
...
void mainImage( out vec4 O, in vec2 g)
{
    ...
    if (uv.y > -.4) //drawing the sky
    {
        ...
        f = mix(sp(sin(length(u) - 0.1)) * 0.35, mix(sp(sin(length(u)-.1)+(hash12(id)-.5)*.15),vec3(1),h), st(abs(lc.x-.5),.4,sm*yd)*st(abs(lc.y-.5),.48,sm*xd));
    };
