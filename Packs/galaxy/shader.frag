...
void main() {
    ...
    #if SCREEN_EFFECT == 1
    if (t < 2.5) {
        float c = (col.r + col.g + col.b) * 0.3 * (0.6 + 0.3 * cos(gl_FragCoord.y * 1.2543)) + 0.1 * (noise((xy + t * 2.0) * 294.0) * noise((xy - t * 3.0) * 321.0));
        c += max(0.0, 0.08 * sin(10.0 * t + xy.y * 7.2543));
        col = vec3(c, c, c) * (1.0 - 0.5 * pow(noise(vec2(t * 99.0, 0.0)), 9.0));
    } else {
        float c = clamp(1.0 - (t - 2.5) * 6.0, 0.0, 1.0);
