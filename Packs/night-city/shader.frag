...
vec3 shade(in vec3 ro, in vec3 pos, in vec3 nor) {
    ...
    if (abs(pos.x) > 15.0 || abs(pos.x) < 8.0) {
        col = vec3(0.02);
    }
    ...
    if (abs(pos.x) > 12.9 && pos.y > 9.0) { // windows
...
vec3 getLightColor(in vec3 pos) {
    vec3 lcol = vec3(1.0, 0.7, 0.5);

    vec3 pd = pos;
    pd.x = abs(pd.x);
    pd.z *= -sign(pos.x);

    float ch = hash(floor((pd.z + 18.0 * time) / 40.0));
    vec3 pdc = vec3(pd.x, pd.y, mod(pd.z + 18.0 * time, 40.0) - 20.0);
...
void main() {
    ...
    // camera
    float z = time;
    float x = -10.9 + 1.0 * sin(time * 0.2);
    vec3 ro = vec3(x, 1.3, z - 1.0);
    vec3 ta = vec3(-8., 1.3 + 0.4 * cos(time * 0.26), z + 4.0 + cos(time * 0.04));
    ...
    if (ints > -0.5) {
        ...
        float r = 0.09;
        if (int1.y > 0.129) {
            r = 0.025 * hash(133.1234 * floor(int1.y / 3.0) + floor(int1.z / 3.0));
        }
    ...
    vec2 st = 256.0 * (p * vec2(0.5, 0.01) + vec2(time * 0.13 - q.y * 0.6, time * 0.13));
