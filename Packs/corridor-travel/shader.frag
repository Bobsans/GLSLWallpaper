...
void main() {
    ...
    for (float j = 0.0; j < steps; j += 1.0) {
        vec2 off = hash22(uv + j * 74.542 + 35.877) * 2.0 - 1.0;
        float t2 = camtime(times + j * 0.05 / steps);
        ...
        for (float i = 0.0; i < 3.0; ++i) {
            ...
            addcol *= smoothstep(0.5 * curve(times + id.y * 0.01 + id.x * 0.03, 0.3), 0.0, hash2(id));
            ...
            addcol += vec3(0.7, 0.5, 1.2) * step(p2.y, -0.9) * max(0.0, curve(times, 0.2) * 2.0 - 1.0) * step(hash2(id + 0.7), 0.2);
            col += addcol * alpha;
