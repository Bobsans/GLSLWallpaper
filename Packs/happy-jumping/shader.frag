...
void main() {
    ...
    float mtime = (time -2.6) * 0.9;

    // camera
    float cl = sin(0.5 * mtime);
    float an = 1.57 + 0.7 * sin(0.15 * mtime);
    vec3  ta = vec3(0.0, 0.65, -0.6 + mtime * 1.0 - 0.4 * cl);
    vec3  ro = ta + vec3(1.3 * cos(an), -0.250, 1.3 * sin(an));
    float ti = fract(mtime - 0.15);
    ti = 4.0 * ti * (1.0 - ti);
    ta.y += 0.15 * ti * ti * (3.0 - 2.0 * ti) * smoothstep(0.4, 0.9, cl);

    // camera bounce
    float t4 = abs(fract(mtime * 0.5) - 0.5) / 0.5;
    float bou = -1.0 + 2.0 * t4;
    ro += 0.06 * sin(mtime * 12.0 + vec3(0.0, 2.0, 4.0)) * smoothstep(0.85, 1.0, abs(bou));
