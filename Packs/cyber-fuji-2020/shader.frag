...
float sun(vec2 uv, float battery) {
    ...
    float cut = 3.0 * sin((uv.y + time * 0.2 * (battery + 0.02)) * 100.0) + clamp(uv.y * 14.0 + 1.0, -6.0, 6.0);
...
float sdCloud(in vec2 p, in vec2 a1, in vec2 b1, in vec2 a2, in vec2 b2, float w) {
    ...
    return min(opSmoothUnion(lineVal1, boxVal, 0.05), opSmoothUnion(lineVal2, boxVal, 0.05));
}
